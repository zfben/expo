...
async function tapExternalRequirePolyfill(projectRoot: string) {
  ...
  await writeIfDifferentAsync(
    ...
    // 'global.$$require_external = typeof expo === "undefined" ? require : () => null;'
    ...
    // 'global.$$require_external = typeof expo === "undefined" ? eval("require") : (moduleId) => { throw new Error(`Node.js standard library module ${moduleId} is not available in this JavaScript environment`);}'
    // 'global.$$require_external = typeof expo === "undefined" ? eval("require") : (moduleId) => { throw new Error(`Node.js standard library module ${moduleId} is not available in this JavaScript environment`);}'
  );
}
